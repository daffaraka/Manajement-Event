            ...
            <div class="card">
                <div class="card-body">
                    @if ($errors->any())
                        ...
                        @endforeach
                    @endif
                    <form action="{{ route('pertanyaan.update', $row) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label>Judul <span class="text-danger">*</span></label>
                            <input class="form-control" type="text" name="pertanyaan_title"
                                value="{{ old('pertanyaan_title', $row->pertanyaan_title) }}" />
                        </div>
                        <div class="form-group">
                            <label>Jawaban <span class="text-danger">*</span></label>
                            <select class="form-control" name="category" />
                            @foreach ($jawabans as $jawaban)
                                @if ($jawaban == old('jawaban', $row->jawaban))
